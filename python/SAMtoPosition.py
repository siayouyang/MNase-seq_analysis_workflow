...
def run():
    ...
    parser.add_option("-s","--sigma", dest="sigma", type="int", default=20,help="Sigma to be used when smoothing. Default %default")
    ...
    parser.add_option("-p", "--processors", dest="processors", type="int", default=5, help="Maximum processors to be run in parallel, will not exceed the maximum chromosomes number. Default %default.")
    (options, args) = parser.parse_args()

    global sam, sigma, exclusion, rawwig, smoothwig, max_processors
    sam = options.sam
    sigma = options.sigma
...
if __name__ == "__main__":
    ...
    merge_info(), append_log("get_info")
